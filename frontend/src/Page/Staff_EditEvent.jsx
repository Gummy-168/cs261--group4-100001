...
export default function StaffEditEventPage({ navigate, auth, data, eventId, requireLogin }) {
  ...
  // รูปภาพ
  const [previewImage, setPreviewImage] = useState(event.imageUrl || null);
  ...
  const handleImageChange = (e) => {
    ...
    if (!file) return;
    setImageFile(file); // ⭐️ เก็บ File object
    const reader = new FileReader();
    reader.onloadend = () => {
      setPreviewImage(reader.result); // ⭐️ เก็บ Base64 ไว้โชว์
      e.target.value = "";
    };
  ...
  // ⭐️⭐️⭐️ [แก้ไข] ฟังก์ชันนี้ทั้งหมด ⭐️⭐️⭐️
  const doSave = async () => {
    try {
      // นี่คือสิ่งที่เราจะส่งไป Backend (เช่น "api/images/file.png" หรือ null)
      let finalRelativePath = null;

      // 1. ตรวจสอบว่ามีการ "เลือกไฟล์ใหม่" หรือไม่
      if (imageFile) {
        toast.loading('กำลังอัปโหลดรูปภาพใหม่...');
        try {
          // [FIX 1] ส่ง "File Object" (imageFile) ไม่ใช่ "String" (imageUrl)
          ...
          return; // หยุดทันทีถ้าอัพโหลดไม่ผ่าน
        }
      } 
      ...
      // finalRelativePath จะเป็น null (ถูกต้อง)

      // 4. สร้าง payload เพื่อส่งไป update
      const payload = {
        ...event,
        ...
        imageUrl: finalRelativePath, // ⭐️ [FIX 4] ส่ง Path ที่ถูกต้อง (หรือ null)
      };
      ...
      toast.dismiss(); // (เผื่อมี toast loading ค้าง)
